...
final class NoteVTests: XCTestCase {
    ...
    func testSessionDataCodableRoundTrip() throws {
        ...
        let decodedFrame = try XCTUnwrap(decoded.frames.first, "Expected at least one frame after decode")
        XCTAssertEqual(decodedFrame.trigger, .periodic)
        XCTAssertEqual(decodedFrame.changeScore, 0.3, accuracy: 0.001)
